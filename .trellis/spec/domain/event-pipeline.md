...
- `alias_candidate` is a proposal with `value`, `kind: name_variant | nickname`,
  ...
  write command.
- `allowLegacyIdentity` is an explicit compatibility switch for trusted legacy
  Event DTO adapters and defaults to `false` in `createRuntime()`. The production
  Analyzer path must not use it to accept an unknown model ID.

#### 3. Contracts

1. Registry entries are keyed only by canonical `character_id`. IDs are opaque,
   generated by Runtime (`crypto.randomUUID()` when available, otherwise the
   dependency-free opaque fallback), and never derived from display names,
   pinyin, romanization, slugs, hashes, translations, initials, or suffixes.
...
| `new` has no reliable existing match | Runtime creates one opaque ID, registers it before the Event is saved, and backfills the current Event |
...
- Good: `祁鸢` exactly matches the unique alias of `char_001`, so the mention
  resolves to `char_001` and no new ID is generated.
- Good: `苏晚` with an explicit first-appearance `new` status receives a Runtime
  opaque ID, the current Event uses it, and the next Analyzer input lists the
  same ID and accepted aliases.
...
- Good: `char_001` and `char_002` both have `晚晚`; exact matching returns both,
...
- Bad: lazy bootstrap sees two historical IDs with the same name and merges,
  redirects, deletes, or rewrites either ID.

#### 6. Tests Required

- Registry unit tests assert opaque ID generation, mutable display names, stable
  IDs, duplicate display/same-sounding/same-alias coexistence, and complete
  candidate sets.
