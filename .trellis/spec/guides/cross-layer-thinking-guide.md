...
### Worldbook Selector Boundary

For a source selector that searches a normalized catalog, keep the complete
source DTO separate from the filtered view. Display filtering may reduce the
visible entry rows, but book-level counts, parent checkbox state, and batch
selection must still use the complete source. A parent checkbox is derived
state, not another persisted selection item.

Before changing this path, verify all three boundaries:

- host `file_id`/entry `uid` becomes stable `source_id`/`entry_id`;
- loader scopes determine the character-mounted/global group;
- UI events write only normalized child selections to Chat-local settings.
