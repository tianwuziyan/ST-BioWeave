...
根据真实 `AnalysisInput` 测试结果，修正 World Model 的生物分类层级，使模型先识别 `species`，再在每个 species 内记录资料实际出现的 `biological_types`，并把 capability 保持在具体 biological type 上。用户重新用同一份输入测试时，不再看到把“人类、妖、剑灵、双性/间性”混为同一级的结果。
...
- 当前 capability 已经是每个 biological type 的字段，但由于 species 层缺失，模型仍可能把 species 与 biological type 混列；`ai/analyzer.js:144-150` 的旧双性/间性过滤也需要适配新的嵌套结构。
...
- 默认人类规则：资料出现男性、女性、双性/间性或其它人类常规身体/生殖分类，且没有明确非人类证据时，可创建 species“人类”；只创建资料实际出现或明确描述的 biological types，不因为“人类”自动补齐男性、女性或双性/间性。
...
- biological type 名称必须是开放的资料分类，不得由 schema 或校验硬编码为男性/女性/双性/间性；应支持 ABO 的 Alpha、Beta、Omega 以及资料定义的其它分类。
...
- [x] 解析一个包含“人类→男性/女性/双性/间性”和“剑灵→男性/女性”的结果时，四个/两个 biological type 分别保存在对应 species 下，capabilities 只存在于各 type。
- [x] 解析结果允许 `人类→Alpha/Beta/Omega` 或其它开放 biological type 名称，不因名称不在固定枚举中而失败；只出现男性时不会自动新增女性或双性/间性。
