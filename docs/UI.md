...
设置页的“世界分析提示词”提供可编辑的顶部 SYSTEM、任务补充、输入前后说明和尾部 SYSTEM；补充内容可以留空。固定生物学约束、输出字段校验和 `AnalysisInput` 的实际资料由 BioWeave 保留；用户人物设定不会进入 World Model 请求。调试预览与请求使用 `可选顶部 SYSTEM → system → system → assistant → user → 可选尾部 SYSTEM` 消息分层：顶部/尾部自定义 SYSTEM、固定约束、角色卡/世界书资料、实际提取出的最近剧情正文、最后分析任务分别显示；空的首尾 SYSTEM 不发送。每段消息都可独立展开查看，不发送格式围栏或完整 schema 代码块，也不在最近剧情正文外附加楼层标题。
