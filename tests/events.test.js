...
test('normalizeEvent emits the fixed source, story time, participants, and relevance shapes', () => {
  ...
  assert.deepEqual(event.pregnancy_relevance.gestational_subject_ids, [
    'char-a',
  ]);
  assert.deepEqual(event.pregnancy_relevance.counterpart_ids, [
    'char-b',
    'char-c',
  ]);
  ...
  assert.equal(
    event.participants[0].reproductive_capabilities_used.can_carry_pregnancy,
    true,
  );
  assert.equal(
    event.participants[0].reproductive_capabilities_used.can_produce_ova,
    null,
  );
});
    ...
    validateEvent(
      validExposureEvent({
        participants: [
          ...
          {
            character_id: 'unrelated-participant',
...
function actualExposureOutcome({
  sourceIds = [],
  possibleConception,
  evidenceText,
}) {
...
test('actual exposure outcome beats protection action and excludes no-path cases', () => {
  const cases = [
    {
      label: 'no barrier with actual exposure',
      possibleConception: true,
      sourceIds: ['character_source'],
      evidenceText: 'actual reproductive substance entered the valid path',
    },
    {
      label: 'barrier break with actual exposure',
      possibleConception: true,
      sourceIds: ['character_source'],
      evidenceText:
        'barrier broke before actual exposure entered the valid path',
    },
    {
      label: 'barrier removal with actual exposure',
      possibleConception: true,
      sourceIds: ['character_source'],
      evidenceText:
        'barrier was removed and actual exposure entered the valid path',
    },
    {
      label: 'barrier slip with actual exposure',
      possibleConception: true,
      sourceIds: ['character_source'],
      evidenceText:
        'barrier slipped and actual exposure entered the valid path',
    },
    {
      label: 'intact barrier without exposure',
      possibleConception: false,
      sourceIds: [],
      evidenceText:
        'intact barrier kept the reproductive substance outside the valid path',
    },
    {
      label: 'external release without valid path',
      possibleConception: false,
      sourceIds: [],
      evidenceText: 'external release did not enter a valid conception path',
    },
    {
      label: 'insertion without reproductive exposure',
      possibleConception: false,
      sourceIds: [],
      evidenceText:
        'insertion occurred without actual reproductive substance exposure',
    },
    {
      label: 'contact without reproductive exposure',
      possibleConception: false,
      sourceIds: [],
      evidenceText:
        'physical contact did not form actual reproductive exposure',
    },
  ];
  for (const outcome of cases) {
    ...
    assert.equal(
      event.pregnancy_relevance.possible_conception,
      outcome.possibleConception,
      outcome.label,
    );
    assert.deepEqual(
...
test('actual exposure outcomes support one or multiple sources without retaining other participants', () => {
  ...
  assert.deepEqual(
    oneSource.participants.map((item) => item.character_id),
    ['character_subject', 'character_source_a'],
  );
  assert.deepEqual(oneSource.pregnancy_relevance.counterpart_ids, [
    'character_source_a',
  ]);
  assert.deepEqual(
    multipleSources.participants.map((item) => item.character_id),
...
test('validateEvent enforces one subject and an exact subject/source participant closure', () => {
  ...
  assert.deepEqual(
    normalizedDuplicateIds.pregnancy_relevance.gestational_subject_ids,
    ['subject'],
  );
  assert.deepEqual(normalizedDuplicateIds.pregnancy_relevance.counterpart_ids, [
    'source-a',
    'source-b',
  ]);
  assert.deepEqual(
    normalizedDuplicateIds.participants.map((item) => item.character_id),
...
test('validateEvent checks raw duplicate participant records at their original indexes', () => {
  ...
  const strictDuplicate = validateEvent(
    validExposureEvent({
      participants: [...baseParticipants, { ...baseParticipants[1] }],
    }),
    { strictCanonicalParticipants: true },
  );
  assert.equal(strictDuplicate.ok, false);
...
test('validateEventCollection rejects duplicate pregnancy subjects without merging Events', () => {
  ...
  const subjectA = exposure({
    eventId: 'evt-a',
    subjectId: 'subject-a',
    counterpartIds: ['source-a'],
  });
  const subjectB = exposure({
    eventId: 'evt-b',
    subjectId: 'subject-b',
    counterpartIds: ['source-b', 'source-c'],
  });
  assert.deepEqual(validateEventCollection([subjectA, subjectB]), {
    ok: true,
    errors: [],
  });
  ...
  const mutualExposure = validateEventCollection([
    exposure({
      eventId: 'evt-mutual-a',
      subjectId: 'subject-a',
      counterpartIds: ['subject-b'],
    }),
    exposure({
      eventId: 'evt-mutual-b',
      subjectId: 'subject-b',
      counterpartIds: ['subject-a'],
    }),
  ]);
  ...
  assert.equal(
    validateEvent(
      validExposureEvent({
        ...
        source_evidence: [
          { kind: 'narrative', text: 'no valid exposure path' },
        ],
      }),
    ).ok,
    true,
  ...
  const events = [
    {
      event_id: 'b',
      source: { floor: 2 },
      story_time: { display: 'later', day_index: 3 },
    },
    {
      event_id: 'a',
      source: { floor: 1 },
      story_time: { display: 'earlier', day_index: 2 },
    },
  ];
