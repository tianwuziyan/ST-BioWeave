...
function renderProfileEditor(profile, draft, testResult, busy, modelList, modelListProfileKey, modelSearch, modelRefreshBusy) {
  ...
  return [
    ...
    '<details class="bioweave-settings-advanced">',
    '<summary>高级设置</summary>',
    '<div class="bioweave-settings-fields">',
    field('Context Size', 'context_size', source.context_size, 'number', ' min="1" step="1" inputmode="numeric"'),
    field('Max Output Tokens', 'max_output_tokens', source.max_output_tokens, 'number', ' min="1" step="1" inputmode="numeric"'),
    field('Temperature', 'temperature', source.temperature, 'number', ' min="0" max="2" step="0.1" inputmode="decimal"'),
    field('Timeout (ms)', 'timeout', source.timeout, 'number', ' min="250" step="250" inputmode="numeric"'),
    field('Retry Count', 'retry_count', source.retry_count, 'number', ' min="0" max="3" step="1" inputmode="numeric"'),
    '</div>',
    '</details>',
...
function renderWorldAnalysisPromptSettings(prompt = {}, openSettingsSections = []) {
  ...
  const labelField = (label, key, value) => [
    '<label class="bioweave-settings-field">',
    '<span>' + label + '</span>',
    '<input class="bioweave-input" type="text" data-bioweave-world-analysis-label="' + key + '" value="' + escapeHtml(value) + '">',
    '</label>',
  ].join('');
  return [
    ...
    renderSettingsSummary('世界分析提示词与标签', '可修改发送给模型的补充内容和分段名称'),
    ...
    '<h4>输入分段名称</h4>',
    '<div class="bioweave-settings-fields">',
    labelField('角色卡', 'character', settings.labels.character),
    labelField('世界书', 'worldbooks', settings.labels.worldbooks),
    labelField('最近剧情', 'recent_story', settings.labels.recent_story),
    labelField('外部记忆', 'external_memory', settings.labels.external_memory),
    '</div>',
    '<div class="bioweave-settings-actions">',
