...
export function renderAnalysisActionButton(value = null, { className = 'bioweave-primary-action' } = {}) {
  ...
  return (
    ...
    '" data-bioweave-action="analyze-current-floor" aria-busy="' +
    String(busy) +
    '">' +
    label +
    '</button>'
...
export function analysisStatusCurrentFloor(value = null, fallback = null) {
  const status = normalizeAnalysisStatus(value)
  return status.current_floor && typeof status.current_floor === 'object' ? status.current_floor : fallback
}
...
function renderSubjectList(trackingSubjects) {
  ...
  return subjects
    ...
    .map(
      ({ id, value: subject }) =>
        '<button type="button" class="bioweave-character-row" data-character-id="' +
        escapeHtml(id) +
        '"><span><b>' +
        escapeHtml(displayValue(subject.display_name, '未命名角色')) +
        '</b></span><span class="bioweave-badge ' +
        trackingSubjectTone(subject.status) +
        '">' +
        (subject.status === 'inactive' ? '已结束' : subject.status === 'active' ? '已追踪' : '未知') +
        '</span>' +
        '<span>›</span></button>',
    )
    .join('')
...
function renderRecentEvents(activeEvents) {
  ...
  return (
    '<div class="bioweave-event-list">' +
    events
      .slice(0, 5)
      .map(event => {
        return (
          '<article class="bioweave-card bioweave-overview-event"><header><b>' +
          escapeHtml(EVENT_TYPE_LABELS[event.type] ?? displayValue(event.type, '生理事件')) +
          '</b><span class="bioweave-badge ' +
          eventStatusTone(event.status) +
          '">' +
          escapeHtml(EVENT_STATUS_LABELS[event.status] ?? displayValue(event.status)) +
          '</span></header><p>发生时间：' +
          escapeHtml(displayValue(storyTimeDisplay(event))) +
          '</p><p>地点：' +
...
export function overviewPage({
  ...
  return (
    ...
    escapeHtml(displayValue(chatName, '当前 Chat')) +
    '</strong>' +
    ' <span class="bioweave-muted">· 当前 Floor ' +
    escapeHtml(displayValue(floor)) +
    ' · Analysis Status：' +
    escapeHtml(analysisStatusLabel(status)) +
    '</span></p></div>' +
    '<div class="bioweave-page-actions">' +
    renderAnalysisActionButton(status) +
    '</div></div>' +
    '<div class="bioweave-summary"><div><strong>' +
    subjectCount +
    '</strong><span>追踪人物</span></div>' +
    '<div><strong>' +
    activeEventCount +
    ...
    analysisStatusTone(status) +
    '">' +
    escapeHtml(analysisStatusLabel(status)) +
    '</span></header>' +
    '<dl class="bioweave-data-list">' +
    '<div><dt>当前 Floor</dt><dd>' +
    escapeHtml(displayValue(floor)) +
    '</dd></div>' +
    '<div><dt>Analysis Status</dt><dd>' +
    escapeHtml(analysisStatusLabel(status)) +
    '</dd></div>' +
    '<div><dt>最近成功分析</dt><dd>' +
    escapeHtml(analysisTimestamp(lastSuccess)) +
    '</dd></div>' +
    '<div><dt>Current Floor Event Count</dt><dd>' +
    eventCount +
    '</dd></div>' +
    '<div><dt>Tracking Subject Count</dt><dd>' +
    subjectCount +
    '</dd></div>' +
    '</dl>' +
    '</section></div></section>'
