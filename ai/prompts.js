...
const WORLD_MODEL_CORE_INSTRUCTIONS = [
  ...
  '【Full / Supplement Shared Review】Full 与 Supplement 都必须先完成 Fact Discovery、species completeness、species/type binding、Exclusion Gate、stable classification、field-level evidence review 和 Unarchived Fact Review。Full 从 permitted evidence 独立建立 complete model，不读取 Existing baseline；Supplement 只在同一 permitted evidence 上额外比较 Existing baseline。Existing 非空不等于完成审查，不能跳过遗漏 species/type、未归档 world fact、capability 或规则分类。Empty Patch 只有在这些 review 全部完成且没有合法 evidence-supported ADD/CHANGE candidate 时才表示无变化；不能仅因为 Existing 已有内容或某个 Existing entry 不便删除就返回空 Patch。',
...
export function buildWorldModelPatchMessages(
  ...
  addMessage(messages, 'user', '请根据 Existing World Model baseline 与以上完整 World Analysis evidence，先完成 World Fact Discovery、species completeness、Species Binding、Exclusion Gate、stable biological classification、field-level evidence review、scope/classification、missing world fact review 与 baseline-aware consolidation，再输出 evidence-supported sparse World Model Patch JSON；Individual-only 或 scope ambiguous 的事实不要进入 World Model，兼容 Existing knowledge 必须保留，Existing baseline/type name 不能作为新增 fact 的 evidence，省略字段按对应 Patch section 的 contract 处理；只有完整 review 后没有合法 ADD/CHANGE candidate 时才返回 empty Patch；只返回 add/update，不返回完整模型。')
