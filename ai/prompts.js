...
const WORLD_MODEL_CORE_INSTRUCTIONS = [
  ...
  'Human baseline 与显式 delta：Human 是唯一内置的现实生物 baseline。只有当前资料支持普通人类背景时才建立“人类”；这种支持可以来自显式 Human/人类，也可以来自完整 AnalysisInput（Character Card、Worldbook、Recent Story、External Memory 和当前上下文）合并后对普通人类默认背景的可靠支持。明确独立 Nonhuman、陌生生命、明显不同身体结构或生理体系、相互冲突的证据或无法判断时禁止 fallback；没有 species、没有 Human 字样、类人外形、男性/女性称谓、性交行为或社会结构本身都不是充分条件，无法可靠判断就保留未知。先判断基础来源，再把当前个体事实、明确转化后/特殊体系规则、明确世界级规则逐 capability、逐 reproduction_rules、逐 lifecycle 字段覆盖 baseline；优先级为明确当前个体事实 > 明确转化后/特殊体系规则 > 明确世界级规则 > 可靠推断的 Human baseline > 未知。delta 只覆盖明确改变的字段，其余稳定字段保留；不适用字段写 null，未成立的 biological_types 保持 []。Human baseline 不创建缺失 type，只适用于已成立的普通 Human Male/Female，不适用于其它 Human type。若资料明确将永久变化后的对象定义为新 species，输出当前 species label 与合成后的当前字段，不新增 source_species、origin 或 inheritance 字段；个体 Human 来源也不能自动扩展为整个新 species 的来源。',
  'Nonhuman 与字段语义：只依据本次 AnalysisInput，不使用模型自身常识，也不从 biological_type 名称套用 Human template；即使类人、使用男性/女性名称、具有类似器官或性交行为，非人类字段仍需该 species 和该 type 的直接证据，不能继承 Human baseline。fertilization 只描述真实受精、授精或配子结合机制，并说明当前 type 的供体/受体角色；性交、体液/能量交换、感染/寄生、侵蚀/异化、身体改造、觉醒、个体生成、力量或关系变化本身都不等于 fertilization。gestation 只描述真实妊娠或孕育过程，非妊娠的身体转化不属于 gestation。lifecycle.maturation 只描述生物成熟或生命阶段变化，aging 只描述寿命、衰老或明确抗衰老生理；职业、修炼、技能、关系或力量 progression 不属于生命周期。资料明确声明与人类相同，也只继承被声明的范围；未知能力、规则或生命周期字段保持 null。',
