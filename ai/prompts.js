...
const WORLD_MODEL_CORE_INSTRUCTIONS = [
  ...
  '输出 JSON 的 key 必须严格保持 schema 规定的英文；除 null、布尔值和数字外，所有字符串值必须使用中文，包括 biological_type 和 sex_category 的 name、description、生殖规则、生命周期、special_rules、exceptions、unknowns、medical_context 及其内部字段的值。',
  '每个 biological_type 都必须包含 sex_categories 数组；每个 sex_category 都必须有中文 name、可为空的 description、capabilities 和 reproduction_rules。默认人类基础类型只包含男性和女性；默认人类至少分别输出“男性”和“女性”两个 sex_category，不能把同一种族的所有性别共用一套能力或生殖规则。',
  '双性/间性不是默认类型或默认 sex_category；只有当本次 AnalysisInput 出现明确的双性/间性身份、身体/生殖特征或世界规则证据时，才在 biological_types 中加入对应类型或在该类型的 sex_categories 中加入对应类别。仅出现“可以双性化”“双性化改造”等可选身体改造规则，不能证明世界存在双性/间性类别；默认男性/女性且没有明确证据时绝不能加入。即使输入资料出现 Homo sapiens、Human 或其它英文标签，输出也必须翻译为中文，不得把这些英文标签写入任何字符串值。',
  '不要仅凭男性/女性/双性/间性标签推断 capabilities，不要默认男性一定产精、女性一定妊娠或双性/间性具备全部能力，也不要套用现实世界固定周期；若资料只呈现默认男性/女性二元、且没有明确非人类证据，则物种识别按人类处理。',
  '双性/间性类型只有在有明确证据并被加入后才参与规则分析；每个 sex_category 的 capabilities 必须逐项依据明确证据独立判断 can_produce_sperm、can_produce_ova、can_be_fertilized、can_fertilize、can_carry_pregnancy；每个能力独立判断，证据不足的单项使用 null，不能因为双性/间性标签自动把所有能力设为 true 或 false。每个 sex_category 的 reproduction_rules 也只记录该性别有证据的规则。',
  '明确非人类证据优先；除上述默认男性/女性二元且无明确非人类证据的情况外，身份未知、一般未知或明确非人类的生物按未知/非人类规则处理，不得仅凭性别、gender、代词、称谓、外貌或身体形态套用人类规则。',
  '资料明确识别为人类，或只呈现默认男性/女性二元且没有明确非人类证据时，才使用人类生殖基线；明确非人类证据优先，不得套用人类基线。此时必须在对应人类 sex_category 的 reproduction_rules 中分别输出排卵（ovulation）、受精（fertilization）、妊娠/孕期（gestation）和分娩/产程周期（labor cycle，字段 labor）的常见人类过程/周期说明。可用约 28 天月经周期中期排卵、妊娠约 40 周（约 280 天，从末次月经起算；约 38 周从受精起算）作为一般基线，受精窗口受排卵影响，产程按阶段描述并保留个体与医疗条件差异；资料未提供具体参数时，在相应字段写明具体参数未知，但这四个基线字段都必须是非空说明，不得为 null。',
  '该人类基线只适用于明确识别的人类类型，以及上述默认男性/女性二元且没有明确非人类证据的资料；对明确非人类类型不得套用，sex_category.capabilities 仍只能依据明确证据填写，绝不能从 gender、性别、代词、称谓、外貌或身体形态推断。',
  ...
  'capabilities 必须使用布尔值或 null，不得通过 gender 推断能力；capabilities 不能从 gender、性别、代词、称谓、外貌或身体形态推断。种族层旧 capabilities/reproduction_rules 只为读取旧 Chat 兼容；新结果把能力和生殖规则放在 sex_categories 内。',
].join('\n')
...
const WORLD_MODEL_OUTPUT_CONTRACT = [
  ...
  'biological_types 中每项包含 name、description、sex_categories、lifecycle、special_rules；sex_categories 必须是数组。',
  'sex_categories 中每项包含 name、description、capabilities、reproduction_rules；description 可以为 null。',
  '每个 sex_category 的 capabilities 固定包含：can_produce_sperm、can_produce_ova、can_be_fertilized、can_fertilize、can_carry_pregnancy；证据不足时使用 null。',
  '每个 sex_category 的 reproduction_rules 固定包含：fertilization、pregnancy_or_carrying、cycle、ovulation、gestation、labor；证据不足时使用 null。',
  '新结果不要把 capabilities 或 reproduction_rules 放回 biological_type 种族层；这两个种族层字段只允许作为旧 Chat 的兼容读取字段保留。',
