...
export const EVENT_ANALYZER_CORE_CONTRACT = [
  ...
  '正式 character_id 形态固定为 char_ 加六位十进制序号（例如 char_000001，最大为 char_999999）；existing 必须复制输入 Registry 提供的 canonical character_id，new 必须返回 character_id:null，绝不能自行生成永久 ID。',
...
const WORLD_MODEL_CORE_INSTRUCTIONS = [
  ...
  '证据：每个 species、biological_type 和字段独立分析；不跨 species 或跨 biological_type 借证据。五个 capability 逐字段独立举证：明确支持才写 true/false，其中 true 需要明确具备证据、false 需要明确不具备证据；未说明、未知或仅凭“通常/一般”不足以判断时写 null，不要把它们写成 false。明确生理性别事实可以作为 biological_type 映射证据之一，但不能单独授权 capability；除已建立的普通人类男性/女性 baseline 外，不从男性、女性、雄性、雌性等名称直接推导能力，匹配的 species/type baseline 或其它明确个体证据才可逐字段确定能力；固定双性统一使用名称“双性”。',
