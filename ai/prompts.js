...
export const EVENT_ANALYZER_CORE_CONTRACT = [
  ...
  'participant capability 判断顺序固定为：先参考 current World Model，再参考 existing character profile，然后核对 Character、Worldbook 与 current narrative evidence；综合完成这条证据链后才填写 reproductive_capabilities_used。biological_context 只用于说明这次 capability 判断所采用的生物身份背景；没有直接人物证据时，未知 capability 必须保持 null，不能根据角色、位置、主动/被动、姓名、外貌或性别补齐完整 capability 套装。',
...
export const EVENT_ANALYZER_OUTPUT_CONTRACT = [
  ...
  `participants 必须是直接相关对象数组；对 sexual_activity 只保留 actual reproductive exposure chain 的 subject 与实际 exposure source，对其它 BiologicalEvent 只保留直接作用对象。每项包含 character_id、display_name、event_role、reproductive_capabilities_used 和 evidence；仅对 event.type === "sexual_activity" 且 pregnancy_relevance.relevant === true、possible_conception === true 的 pregnancy-related Event，每个 participant 还必须包含 biological_context 对象，固定包含 species 与 biological_type 两个字段，值只能是非空字符串或 null，未知填 null。species 来自当前 World Model；biological_type 是该 species 下稳定的生理/生殖分类。不得新增 gender，也不得按姓名、外貌、event_role、性行为位置、主动/被动或性别推断身份。event_role 只能取：${EVENT_REPRODUCTIVE_ROLES.join('、')}。它表示本事件中的生殖角色，不表示姿势、主动/被动、攻/受、职业、性别或社会角色。`,
  `capability 判断顺序固定为：current World Model → existing character profile → Character / Worldbook / current narrative evidence；综合证据后才填写 reproductive_capabilities_used。没有直接人物证据时，未知 capability 必须保持 null；biological_context 仅说明本次能力判断采用的生物身份背景，不能用角色、位置、主动/被动、姓名、外貌或性别替代证据。reproductive_capabilities_used 固定包含 ${EVENT_CAPABILITY_KEYS.join('、')}；每个值只能是 true、false 或 null。`,
...
const WORLD_MODEL_CORE_INSTRUCTIONS = [
  ...
  '临时状态与内部自检：临时、可逆或条件性的性征、器官或生殖能力变化留在已有 type 的规则/例外中，不能建立新的 biological_type。输出前进行内部自检（不要输出过程）：对每个候选执行 A–E；完成每个 species 的 biological_types 后，再回扫该 species 在完整 AnalysisInput 中由数量、频率、对比、并存或例外语义明确指向但尚未输出的稳定生理/生殖分类，逐个重新执行 A–E，能唯一回指且通过才补入；不得因为已有一个 type、类型数量少或常识认为存在配对 type 而补全。即使只有一个候选，也要额外确认它是独立且稳定的 biological classification，不能因数量为一自动保留；再检查每个非 null capability 的直接依据、fertilization 是否真的描述受精、lifecycle 是否真的描述生命周期。没有可靠答案就把字段降为 null 或删除错误 type。medical_context、exceptions、unknowns 只记录资料明确支持的内容。',
].join('\n')
