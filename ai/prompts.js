...
const WORLD_MODEL_CORE_INSTRUCTIONS = [
  ...
  '输出 JSON 的 key 必须严格保持 schema 规定的英文；除 null、布尔值和数字外，所有字符串值必须使用中文，包括 name、description、生殖规则、生命周期、special_rules、exceptions、unknowns、medical_context 及其内部字段的值。',
  '默认人类基础类型只包含男性和女性；双性/间性不是默认类型，只有当本次 AnalysisInput 出现明确的双性/间性身份、身体/生殖特征或规则证据时，才在 biological_types 中加入对应类型；默认男性/女性且没有明确双性/间性证据时，绝不能生成双性/间性类型。所有类型的 name 只能写“人类”“男性”“女性”“双性/间性”等中文名称；即使输入资料出现 Homo sapiens、Human 或其它英文标签，输出也必须翻译为中文，不得把这些英文标签写入任何字符串值。',
  '不要仅凭男性/女性/双性/间性标签推断 capabilities，不要默认男性一定产精、女性一定妊娠或双性/间性具备全部能力，也不要套用现实世界固定周期；若资料只呈现默认男性/女性二元、且没有明确非人类证据，则物种识别按人类处理。',
  '双性/间性类型只有在有明确证据并被加入后才参与规则分析；它不是全部生殖能力的结论。必须逐项依据明确证据判断 can_produce_sperm、can_produce_ova、can_be_fertilized、can_fertilize、can_carry_pregnancy，每个能力独立判断，证据不足的单项使用 null，不能因为双性/间性标签自动把所有能力设为 true 或 false。',
