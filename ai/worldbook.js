...
export const DEFAULT_WORLD_BOOK_SELECTION_EXCLUSIONS = Object.freeze(['状态', '手机', '美化', 'NSFW', 'cot', '玩法', '超雄', '思维链'])
...
function worldbookCacheFor(options = {}) {
  ...
  const owner =
    typeof options.fetchRef === 'function' ? options.fetchRef : options.context && typeof options.context === 'object' ? options.context : null
...
function runtimeWorldbookReference(value) {
  ...
  const hostKey =
    firstRuntimeReferenceText(value, ['host_key', 'hostKey', 'file_id', 'fileId', 'worldbook_id', 'worldbookId', 'lorebook_id', 'lorebookId']) ||
    (!sourceId ? firstRuntimeReferenceText(value, ['id', 'key']) : '')
  const name =
    firstRuntimeReferenceText(value, [
      'name',
      'label',
      'worldbook_name',
      'worldbookName',
...
function sourceContent(value, seen = new Set()) {
  ...
  if (Array.isArray(value))
    return value
      .map(item => sourceContent(item, seen))
      .filter(Boolean)
      .join('\n')
  const preferredKeys = [
    'content',
...
function entryIdFrom(value, hostKey) {
  if (value && typeof value === 'object') {
    for (const key of ['entry_id', 'uid', 'id', 'uuid', 'key_id']) {
      ...
      const candidate = typeof rawId === 'string' || typeof rawId === 'number' ? String(rawId).trim() : ''
...
function metadataStringArray(value) {
  return (Array.isArray(value) ? value : []).map(metadataString).filter(Boolean)
}
function entryMetadataFrom(value) {
  const source = value && typeof value === 'object' ? value : {}
  const nested = source.metadata && typeof source.metadata === 'object' ? source.metadata : {}
  return {
    comment: metadataString(source.comment ?? nested.comment),
    keys: metadataStringArray(source.keys ?? source.key ?? nested.keys),
    secondary_keys: metadataStringArray(source.secondary_keys ?? source.keysecondary ?? nested.secondary_keys),
  }
}
export function normalizeWorldbookEntries(rawEntries) {
  const seen = new Set()
  return entryCandidates(rawEntries)
    .map(({ value, hostKey, index }) => {
      const entryId = entryIdFrom(value, hostKey)
      if (!entryId || seen.has(entryId)) return null
      seen.add(entryId)
      const content = value && typeof value === 'object' ? (value.content ?? value.text ?? '') : value
...
export function detectCurrentCharacterGreetingField(character, context) {
  ...
  if (mainGreeting) {
    ...
    return sourceChildText(alternateGreetings[alternateIndex]) ? `${CHARACTER_CARD_OPENING_PREFIX}alternate:${alternateIndex}` : ''
  }
  return sourceChildText(alternateGreetings[swipeId]) ? `${CHARACTER_CARD_OPENING_PREFIX}alternate:${swipeId}` : ''
}
...
function fieldsFromSource(source) {
  ...
  return source.fields
    ...
    .filter(
      field =>
        field.field_key && (field.field_key === 'description' || isCharacterCardOpeningFieldKey(field.field_key)) && field.label && field.content,
    )
}
export function normalizeAnalysisSource(raw = {}, overrides = {}) {
  ...
  const sourceId =
    textValue(overrides.source_id ?? source.source_id) || (sourceType === ANALYSIS_SOURCE_TYPES.WORLDBOOK ? sourceIdForWorldbook(hostKey) : '')
  const content = overrides.content ?? source.content ?? source.text ?? ''
  const entries =
    sourceType === ANALYSIS_SOURCE_TYPES.WORLDBOOK ? entriesFromSource({ ...source, entries: overrides.entries ?? source.entries }, content) : []
  const fields =
    sourceType === ANALYSIS_SOURCE_TYPES.CHARACTER_CARD
      ...
      : []
  const contentTokenEstimate =
    sourceType === ANALYSIS_SOURCE_TYPES.WORLDBOOK
...
function mergeSource(existing, next) {
  const content = existing.content_available || existing.content_loaded || existing.entries.length ? existing.content : next.content
  ...
  const tokenEstimateValue =
    type === ANALYSIS_SOURCE_TYPES.WORLDBOOK
      ? sameEntries
        ? existing.token_estimate
        : entries.reduce((total, entry) => total + Number(entry.token_estimate || 0), 0)
...
export function mergeAnalysisSources(sources = []) {
  ...
  for (const source of sources) {
    ...
    const mergeKey =
      normalized.source_type === ANALYSIS_SOURCE_TYPES.WORLDBOOK && normalized.host_key ? `worldbook:${normalized.host_key}` : normalized.source_id
...
function childMatches(child, query, key) {
  return [child?.[key], child?.label, child?.content].some(value =>
    String(value ?? '')
      .toLocaleLowerCase()
      .includes(query),
  )
...
export function searchAnalysisSources(sources = [], query = '') {
  ...
  return sources
    .map(source => {
      const sourceMatches = [source.label, source.source_id, source.source_type, ...(source.scopes ?? [])].some(value =>
        String(value ?? '')
          .toLocaleLowerCase()
          .includes(normalizedQuery),
      )
      if (sourceMatches) return source
      const entries = Array.isArray(source.entries) ? source.entries.filter(entry => childMatches(entry, normalizedQuery, 'entry_id')) : []
      const fields = Array.isArray(source.fields) ? source.fields.filter(field => childMatches(field, normalizedQuery, 'field_key')) : []
...
function selectionIdentity(item) {
  const normalized = typeof item === 'string' ? { source_id: textValue(item), enabled: true } : normalizedSelectionItem(item)
...
function normalizedSelections(selected = []) {
  ...
  for (const item of Array.isArray(selected) ? selected : []) {
    const normalized =
      typeof item === 'string' ? (textValue(item) ? { source_id: textValue(item), enabled: true } : null) : normalizedSelectionItem(item)
...
export function isDefaultWorldbookEntryExcluded(entry = {}, exclusions = DEFAULT_WORLD_BOOK_SELECTION_EXCLUSIONS) {
  const metadata = entry?.metadata && typeof entry.metadata === 'object' ? entry.metadata : entryMetadataFrom(entry)
  const values = [
    metadata.comment,
  ...
  return values.some(value =>
    (Array.isArray(exclusions) ? exclusions : DEFAULT_WORLD_BOOK_SELECTION_EXCLUSIONS).some(keyword =>
      defaultSelectionKeywordMatches(value, keyword),
    ),
  )
}
export function buildInitialWorldbookSelection(sources = []) {
  ...
  for (const source of Array.isArray(sources) ? sources : []) {
    ...
    if (source.source_type !== ANALYSIS_SOURCE_TYPES.WORLDBOOK || source.worldbook_group !== WORLD_BOOK_RUNTIME_GROUPS.CHARACTER_CARD) continue
    for (const entry of Array.isArray(source.entries) ? source.entries : []) {
      const entryId = textValue(entry?.entry_id)
...
export function sourceSelectionStats(sources = [], selected = []) {
  ...
  return {
    ...
    worldbook_count: [...selectedSources].filter(sourceId =>
      sources.some(source => source.source_id === sourceId && source.source_type === ANALYSIS_SOURCE_TYPES.WORLDBOOK),
    ).length,
    token_estimate: tokenEstimateValue,
...
export function updateSourceSelection(selected = [], target, enabled) {
  ...
  const item = typeof target === 'string' ? { source_id: textValue(target), enabled: true } : normalizedSelectionItem({ ...target, enabled: true })
  const identity = selectionIdentity(item)
...
export function shouldEagerLoadWorldbookSource(source = {}) {
  return source?.source_type === ANALYSIS_SOURCE_TYPES.WORLDBOOK && source?.worldbook_group === WORLD_BOOK_RUNTIME_GROUPS.CHARACTER_CARD
}
...
function sourceRuntimeNames(source) {
  return [source?.label, source?.name, source?.host_key].map(value => textValue(value).toLocaleLowerCase()).filter(Boolean)
}
...
function sourceMatchesAnyRuntimeReference(source, references, nameCounts) {
  return (Array.isArray(references) ? references : []).some(reference => sourceMatchesRuntimeReference(source, reference, nameCounts))
}
function characterExtensionWorldbookReferences(character) {
  const extensions = character?.data?.extensions ?? character?.extensions ?? {}
  const additional =
    extensions.additional_worldbooks ??
...
function chatWorldbookReferences(context) {
  ...
  for (const value of metadataValues) {
    ...
    const chatValues = value?.chat ?? value?.chat_worldbooks ?? value?.chatWorldbooks ?? value?.names ?? value?.worldbooks ?? value?.books
...
async function resolveWorldbookRuntimeReferences(context, options = {}) {
  ...
  return {
    ...
    character: uniqueRuntimeWorldbookReferences([...characterWorldbooks.primary, ...characterWorldbooks.additional]),
...
function runtimeWorldbookGroupForSource(source, references, nameCounts) {
  ...
  const selectedGlobal = sourceMatchesAnyRuntimeReference(source, references?.global, nameCounts) || scopes.has('selected_global_worldbook')
  const characterCard = sourceMatchesAnyRuntimeReference(source, references?.character_primary, nameCounts) || scopes.has('character_card_worldbook')
  const character = sourceMatchesAnyRuntimeReference(source, references?.character_additional, nameCounts) || scopes.has('character_worldbook')
  const chat = sourceMatchesAnyRuntimeReference(source, references?.chat, nameCounts) || scopes.has('chat_worldbook')
...
function classifyWorldbookSource(source, references, nameCounts) {
  ...
  const scopes = new Set(Array.isArray(source.scopes) ? source.scopes : [])
  const characterCardMatch =
    sourceMatchesAnyRuntimeReference(source, references?.character_primary, nameCounts) || scopes.has('character_card_worldbook')
  const characterMatch = sourceMatchesAnyRuntimeReference(source, references?.character_additional, nameCounts) || scopes.has('character_worldbook')
  const chatMatch = sourceMatchesAnyRuntimeReference(source, references?.chat, nameCounts) || scopes.has('chat_worldbook')
  ...
  const loadedSources = await Promise.all(
    scopedSources.map(source =>
      source.worldbook_group === WORLD_BOOK_RUNTIME_GROUPS.CHAT
        ...
        : shouldEagerLoadWorldbookSource(source) || options.deferWorldbookContent !== true || worldbookSourceMatches(source, selectedIds)
...
function mergeCharacterWorldbookSources(globalSources, characterReferences = {}) {
  ...
  for (const [referenceKey, scope] of referenceGroups) {
    ...
    for (const reference of references) {
      ...
      const sourceId = textValue(existing?.source_id) || textValue(reference?.source_id) || sourceIdForWorldbook(hostKey)
      ...
      const normalized = isNormalizedSource(base)
        ? copyNormalizedSource(base, {
            source_id: sourceId,
            source_type: ANALYSIS_SOURCE_TYPES.WORLDBOOK,
            host_key: hostKey || base.host_key || null,
            scopes,
          })
...
export async function loadAnalysisSources(options = {}) {
  ...
  const selectedIds = new Set(options.loadContentForSourceIds ?? [])
  const characterWorldbookSources = await Promise.all(
    mergeCharacterWorldbookSources(globalSources, {
  ...
  const mergedSources = mergedPreliminarySources
    ...
    .filter(source => source.source_type !== ANALYSIS_SOURCE_TYPES.WORLDBOOK || source.worldbook_group !== WORLD_BOOK_RUNTIME_GROUPS.CHAT)
